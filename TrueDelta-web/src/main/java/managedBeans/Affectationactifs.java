...
@ManagedBean(name = "affectationactif")
@SessionScoped
public class Affectationactifs implements Serializable {
...
private List<String> devises = Arrays.asList("euro", "dollar", "dinar", "dollarcanadien", "yen",
	...
	public Affectationactifs() {}


	public String doAffecterTA() throws IOException {
		Transaction transaction = ts.getTransactionById(id);
		ActifFinancier actif = ts.getActifById(ida);
		    ...
		    transaction.setRendementTransaction((float) ((solde* 1)/ 100));
		
			
		

	ts.affecterActifTransaction(ida,id);
	return "accueil?faces-redirect=true";
	}
	
	
	public String doAffecterCA() throws IOException {
		Transaction trans = ts.getTransactionById(id);
		ActifFinancier actif = ts.getActifById(ida);
	   Compte compt = ts.getCompteById(idC);
	   
	    ComptePK idC =new ComptePK();
		...
		ts.affecterTransactionCompte(idC, ida, id, devisecompte);
	return "accueil?faces-redirect=true";
	}
	@PostConstruct
	public void init() {
