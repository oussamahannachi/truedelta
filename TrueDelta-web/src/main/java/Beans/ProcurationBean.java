...
public class ProcurationBean {
	...
	public void CurrentUser() {
		idc=lb.getUser().getId();
		//idc = 4;
		System.out.println("Current user : " + idc); 
		Courtier = procurationService.nomCourtier(idc);
		System.out.println("Current user name : " + Courtier);
		procuration_client = procurationService.getProcByClient(idc);
		procuration_courtier = procurationService.getProcByCourtier(idc);
	}
	...
	public String AddProcuration() throws ParseException, MalformedURLException, IOException, DocumentException {
		...
		idc = 5;
		int idcr = 2;
		ProcurationPK pk = new ProcurationPK(idc, idcr);
		Client client=procurationService.getEm().find(Client.class, idc);
		...
		p.setDateCreation(d);
		if (procurationService.VerifierExistanceProc(idc, idcr) == 1) {
